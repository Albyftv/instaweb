...
export default function ServicesTemplate({ business: b }: { business: Business }) {
  ...
  const waLink = wa
    ? `https://wa.me/${wa.startsWith('34') ? wa : `34${wa}`}?text=Hola%2C+me+gustar%C3%ADa+pedir+informaci%C3%B3n+sobre+${encodeURIComponent(b.name)}`
    : null
    ...
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <header className="relative overflow-hidden"
        ...
        <div className="relative z-10 max-w-2xl mx-auto px-6 py-16 text-center">
          <div className="inline-block bg-white/20 backdrop-blur-sm border border-white/30 rounded-full px-4 py-1 text-white/90 text-xs font-semibold tracking-widest uppercase mb-6">
            Servicios profesionales
          </div>
          <h1 className="text-4xl sm:text-5xl font-black text-white leading-tight mb-3">{b.name}</h1>
          {b.tagline && <p className="text-white/80 text-lg max-w-md mx-auto">{b.tagline}</p>}
        </div>
      </header>

      <div className="max-w-2xl mx-auto px-6 -mt-6 pb-32 space-y-4">

        {/* ── ACCIONES ───────────────────────────────────────── */}
        <div className="flex gap-3">
          {waLink && (
            <a href={waLink} target="_blank" rel="noopener"
              className="flex-1 flex items-center justify-center gap-2 bg-white text-gray-900 font-bold py-4 rounded-2xl text-sm shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all">
              <IconWhatsApp />
              WhatsApp
            </a>
          )}
        ...
        {b.description && (
          <div className="bg-white rounded-3xl p-6 shadow-sm">
            <p className="text-gray-600 leading-relaxed text-base">{b.description}</p>
          </div>
        )}

        {/* ── INFO DE CONTACTO ──────────────────────────────── */}
        <div className="bg-white rounded-3xl p-6 shadow-sm space-y-4">
          <h2 className="text-xs font-bold tracking-widest uppercase text-gray-400">Contacto</h2>
      ...
      {/* ── FOOTER ─────────────────────────────────────── */}
      <footer className="fixed bottom-0 left-0 right-0 z-0 text-center py-3">
