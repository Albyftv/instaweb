...
export default function ShopTemplate({ business: b }: { business: Business }) {
  ...
  const waLink = wa
    ? `https://wa.me/${wa.startsWith('34') ? wa : `34${wa}`}?text=Hola%2C+tengo+una+consulta+sobre+${encodeURIComponent(b.name)}`
    : null
  ...
  return (
    <div className="min-h-screen bg-white" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <header className="relative overflow-hidden"
        style={{ background: `linear-gradient(135deg, ${b.primary_color} 0%, ${b.accent_color} 100%)` }}>
        {/* Diagonal stripe decoration */}
        <div className="absolute right-0 top-0 h-full w-1/3 opacity-10"
          style={{ background: 'repeating-linear-gradient(-45deg, white, white 2px, transparent 2px, transparent 12px)' }} />
        <div className="relative z-10 max-w-2xl mx-auto px-6 py-14">
          <div className="inline-block bg-white/20 border border-white/30 rounded-full px-3 py-1 text-white/80 text-xs font-bold tracking-widest uppercase mb-4">
            Tienda
          </div>
          <h1 className="text-4xl sm:text-5xl font-black text-white leading-tight">{b.name}</h1>
          {b.tagline && <p className="text-white/80 text-lg mt-2">{b.tagline}</p>}
        </div>
      </header>

      <div className="max-w-2xl mx-auto px-6 py-8 pb-32 space-y-4">
        ...
        <div className="flex gap-3">
          {waLink && (
            <a href={waLink} target="_blank" rel="noopener"
              className="flex-1 flex items-center justify-center gap-2 bg-gray-900 text-white font-bold py-4 rounded-2xl text-sm shadow-lg hover:bg-gray-800 transition-colors">
              <IconWhatsApp />
              WhatsApp
            </a>
          )}
          {b.phone && (
            <a href={`tel:${b.phone}`}
              className="flex-1 flex items-center justify-center gap-2 text-white font-bold py-4 rounded-2xl text-sm shadow-lg hover:opacity-90 transition-opacity"
        ...
        <div className="border border-gray-100 rounded-3xl overflow-hidden">
          {b.address && (
            <a href={mapsLink ?? '#'} target={mapsLink ? '_blank' : undefined} rel="noopener"
              className="flex items-start gap-4 p-5 border-b border-gray-50 hover:bg-gray-50 transition-colors group">
          ...
          {b.phone && (
            <a href={`tel:${b.phone}`} className="flex items-center gap-4 p-5 border-b border-gray-50 hover:bg-gray-50 transition-colors group">
          ...
          {b.email && (
            <a href={`mailto:${b.email}`} className="flex items-center gap-4 p-5 hover:bg-gray-50 transition-colors group">
              <span className="flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center text-white"
                style={{ background: b.primary_color }}>
                <IconMail />
              </span>
        ...
        {(b.instagram || b.facebook) && (
          <div>
            <h2 className="text-xs font-bold tracking-widest uppercase text-gray-400 mb-3">Síguenos</h2>
      ...
      <footer className="text-center py-4">
        <p className="text-xs text-gray-300">
          Creado con <a href="https://instaweb.es" className="hover:text-gray-500">instaweb.es</a>
        </p>
      </footer>
